...
def cross_hyperparameter_search_panel():
    """
    Plots a 3 panels figure on 1 rows x 3 columns
    Rows contains figures representing cross hyperparameters search for 10
    permutedMNIST tasks resulting from cross_hyperparameter_search.py config
    file.
    Columns 1 is the number of dendritic segments, columns 2 the activation
    sparsity and column 3 the weight sparsity.
    """
    # os.getcwd()
    # os.chdir('/home/jeremy/nta/nupic.research/projects/dendrites/permutedMNIST/hyperparameter_search')
    df_path1 = f"{experiment_folder}cross_search_lasttask.csv"
    ...
    df1bis = df1.loc[(df1["Num segments"] == 10) & (df1.LR == 0.005), ]
    X = np.array(df1bis["Activation sparsity"])
    Y = np.array(df1bis["FF weight sparsity"])
    Z = df1bis["Accuracy"]
    # Z = np.array([np.array(df1bis["Accuracy"])])
...
if __name__ == "__main__":
    ...
    experiment_folder = "data_cross_hyperparameter_search/"

    cross_hyperparameter_search_panel()
