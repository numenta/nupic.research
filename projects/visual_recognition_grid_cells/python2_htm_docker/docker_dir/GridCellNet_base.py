...
class PIUNCorticalColumnForVisualRecognition(PIUNCorticalColumn):
  ...
  def getCopyVersionLocationRepresentation(self):
    """
    ...
    for module in self.L6aModules:
      ...
      activeCells_list.append(module.getActiveCells())

    return activeCells_list
...
class PIUNExperimentForVisualRecognition(PIUNExperiment):
  ...
  def inferObjectWithRandomMovements(self,
    ...
    @return inferredStep (int or None), incorrect (dic), prediction_sequence (list), touchSequence (list)
    """
    self.reset()
    ...
    incorrect = {'never_converged':1,'false_convergence':0} #Track if the non-recognition was due to convergance to an incorrect 
    # representation or never converging
    ...
    for _ in xrange(self.maxTraversals):
      ...
      while True:
        ...
        if fixed_touch_sequence==None:
          ...
          print("\nPerforming inference using an arbitrary, unfixed sequense of touches:")
          print(touchSequence)
        
        else:
          print("\nPerforming inference using a fixed random sequense of touches:")
          touchSequence = fixed_touch_sequence
          print(touchSequence)
      ...
      prediction_sequence = [] #contains a list of the current SDR prediction, as well as 
      # previously sensed input SDRs up until inference is successful
      ...
      for iFeature in touchSequence:
        ...
        pre_touch_location_list = self.column.getCopyVersionLocationRepresentation() # Save representation for later

        featureSDR = self.features[feature["name"]]

        self._sense(featureSDR, learn=False, waitForSettle=False)

        predictedColumns = map(int, list(set(np.floor(self.column.L4.getBasalPredictedCells()/cellsPerColumn))))
        # Note _sense of the feature itself does not change the predicted columns on this touch iteration
        # (and thus does not invalidate the prediction), but it does ensure the BasalPredictedCells have been 
        # updated following the movement, and we re-set the location representation later once in post-inference 

        # Include all previously sensed/predicted representaitons by over-writing current_sequence
        current_sequence = sense_sequence[:]

        current_sequence.append(list(predictedColumns)) #include the newly predicted columns
        ...
        if not inferred: 
          ...
          # Re-set location representations after inference successful so that additional sensations don't influence predictions, 
          # and we can use the output predictions to visualize what the network sees across the rest of the input space
          module_iter = 0
          for module in self.column.L6aModules:
            module.activeCells = pre_touch_location_list[module_iter]
            module_iter += 1

          #Once inference has taken place, sense_sequence gathers predictions
          sense_sequence.append(list(predictedColumns))


        if not inferred:
          # Use the sensory-activated cells to detect whether the object has been
          # recognized. If these sensory-activated cells
          ...
          if len(set(representation)) > 0:
            ...
            for target_iter in range(10):
              target_representations = allClassTargets[target_iter][iFeature]

              if (set(representation) <= target_representations):
                classification_list.append(target_iter)
                print("Classified as a " + str(target_iter) + " with ground truth of " + objectDescription['name'])

          if len(classification_list) > 1:
            print("Classification list : classified as multiple classes!")
            print(classification_list)

            print("***Provisional code to handle multiple classes implemented, but as never experienced, the reliability of the intersection resolution is untested***")
            exit()
            ...
            for ambiguous_class_iter in range(len(classification_list)):
              ...
              intersection_list.append(len(set(representation).intersection(allClassTargets[classification_list[ambiguous_class_iter]][iFeature])))

            # *** note this does not deal with Numpy's behaviour if two locations have the 
            # same maximum value - see https://numpy.org/doc/stable/reference/generated/numpy.argmax.html

            print("Intersection list:")
            print(intersection_list)
            classification_list = classification_list[np.armax(intersection_list)] 
          ...
          if inferred:
            if classification_list[0] == int(objectDescription['name'][0]):
              ...
              plt.imsave('misclassified/trial_' + str(trial_iter) + '_example_' + objectDescription["name"] + '_converged_to_' + str(classification_list[0]) + '.png', objectImage)
              return None, incorrect, prediction_sequence, touchSequence

        finished = ((((inferred and numSensations is None) or
                    (numSensations is not None and currentStep == numSensations))) and currentStep == 25) # Continuing to 
          # step 25 ensures we gather network predictions even after inference is successful
