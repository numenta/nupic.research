...
def chart():
    ...
    for cpm, marker in zip((100, 196, 400), markers):
        with open(
            "results/capacity_1_500_feats_{}_cpm.json".format(str(cpm)), "r"
        ) as f:
        ...
        for exp in experiments:
            ...
            expResults.append((numObjects, 1.0 - (float(failed) / float(numObjects))))
        ...
        plt.plot(
            ...
            y,
            "{}-".format(marker),
            label="{} Cells Per Module".format(str(cpm)),
        )
    ...
    for feats, marker in zip((100, 200, 500), markers):
        with open(
            "results/capacity_{}_feats_400_cpm.json".format(str(feats)), "r"
        ) as f:
            experiments = json.load(f)
        expResults = []
        for exp in experiments:
        ...
        plt.plot(
            ...
            y,
            "{}-".format(marker),
            label="{} Unique Features".format(str(feats)),
        )
