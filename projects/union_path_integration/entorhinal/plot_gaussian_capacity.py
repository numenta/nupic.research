...
def varyResolution_varyNumModules(
    inFilenames,
    outFilename,
    resolutions=(2, 3, 4),
    moduleCounts=(
    ...
    maxNumObjectsByResolution=None,
):
    if maxNumObjectsByResolution is None:
...
def varyModuleSize_varyResolution(
    inFilenames,
    outFilename,
    scalingFactors=(1, 2),
    resolutions=(2, 3, 4),
    maxNumObjectsByParams=None,
):
    if maxNumObjectsByParams is None:
    ...
    leg = ax1.legend(
        ...
        frameon=False,
        handles=[matplotlib.lines.Line2D([], [], color=color) for color in colors],
        labels=["$ \\sigma = \\sigma_{rat} $", "$ \\sigma = \\sigma_{rat} / 2.0 $"],
    )
...
def varyModuleSize_varyNumModules(
    inFilenames,
    outFilename,
