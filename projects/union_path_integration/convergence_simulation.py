...
from nupic.research.frameworks.location.two_layer_tracing import PIUNLogger as rawTrace
from nupic.research.frameworks.location.two_layer_tracing import PIUNVisualizer as trace
...
class PIUNCellActivityTracer(PIUNExperimentMonitor):
    ...
    def afterLocationAnchor(self, **kwargs):
        moduleStates = [
            {"activeCells": module.activeCells.tolist()}
        ...
        self.locationLayerTimelineByObject[self.currentObjectName].append(moduleStates)
...
def doExperiment(
    ...
    objects = generateObjects(
        numObjects, featuresPerObject, objectWidth, numFeatures, featureDistribution
    )

    if logNumFeatureOccurrences:
        featureOccurrences = collections.Counter(
            feat["name"] for obj in objects for feat in obj["features"]
        )
    ...
    perModRange = float((90.0 if bumpType == "square" else 60.0) / float(numModules))
    ...
    try:
        ...
        for objectDescription in objects:
            ...
            if logNumFeatureOccurrences:
                objectFeatureOccurrences = sorted(
                    featureOccurrences[feat["name"]]
    ...
    for step, num in sorted((k, v) for k, v in convergence.items() if k is not None):
...
def runMultiprocessNoiseExperiment(
    resultName, repeat, numWorkers, appendResults, **kwargs
):
    """
...
def runExperiments(experiments, resultName, numWorkers=-1, appendResults=False):
    ...
    if numWorkers > 1:
        ...
        while not rs.ready():
            ...
            print(
                "    =>", remaining, "experiments remaining, percent complete=", pctDone
            )
            time.sleep(5)
...
def main():
    ...
    parser.add_argument(
        "--noiseFactor", type=float, nargs="+", required=False, default=0
    )
    parser.add_argument(
        "--moduleNoiseFactor", type=float, nargs="+", required=False, default=0
    )
    ...
    cellCoordinateOffsets = tuple(
        [i * (0.998 / (numOffsets - 1)) + 0.001 for i in range(numOffsets)]
    )
    ...
    runMultiprocessNoiseExperiment(
        ...
        numSensations=(args.numSensations if args.numSensations != -1 else None),
