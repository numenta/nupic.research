...
def doExperiment(
    ...
    objects = generateObjects(numObjects, featuresPerObject, objectWidth, features)
    ...
    if anchoringMethod == "corners":
        cellCoordinateOffsets = (0.0001, 0.5, 0.9999)
    ...
    for objectDescription in objects:
        exp.learnObject(
            objectDescription, randomLocation=randomLocation, useNoise=False
        )
        ...
        with io.open(filename, "w", encoding="utf8") as fileOut:
            with trace(fileOut, exp, includeSynapses=False):
                ...
                for objectDescription in objects:
                    ...
                    if steps is None:
                        print(
                            'Failed to infer object "{}"'.format(
                                objectDescription["name"]
                            )
                        )
                    else:
                        print(
                            "Inferred object {} after {} steps".format(
...
def runMultiprocessNoiseExperiment(resultName=None, numWorkers=0, **kwargs):
    ...
    if numWorkers > 1:
        ...
        while not rs.ready():
            ...
            print(
                "    => {} experiments remaining, percent complete={}".format(
                    remaining, pctDone
                )
            )
            time.sleep(5)
