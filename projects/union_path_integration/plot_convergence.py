...
def chart():
    ...
    for feats, marker in zip((100, 200, 5000), markers):
        with open(
            "results/convergence_vs_num_objs_{}_feats.json".format(feats), "r"
        ) as f:
        ...
        y = [
            float(sum(pair[1])) / float(len(pair[1])) if None not in pair[1] else None
            for pair in yData
        ]
        ...
        plt.plot(
            ...
            y,
            "{}-".format(marker),
            label="{} unique features".format(feats),
        )
    ...
    for cpm, marker in zip((49, 100, 400), markers):
        with open(
            "results/convergence_vs_num_modules_100_feats_{}_cpm.json".format(cpm), "r"
        ) as f:
    ...
    with open(
        "results/cumulative_convergence_1600_cpm_10_feats_100_objs.json", "r"
    ) as f:
    ...
    with open(
        "results/cumulative_convergence_400_cpm_10_feats_100_objs.json", "r"
    ) as f:
        experiments = json.load(f)
    for exp in experiments:
        cum = 0
    ...
    with open(
        "results/cumulative_convergence_289_cpm_10_feats_100_objs_1.json", "r"
    ) as f:
        experiments = json.load(f)
    for exp in experiments:
        cum = 0
