...
OBJECTS = [
    {
        "name": "Object 1",
        "features": [
            {"top": 0, "left": 0, "width": 10, "height": 10, "name": "A"},
    ...
    with io.open(filename, "w", encoding="utf8") as fileOut:
        with trace(fileOut, exp, includeSynapses=True):
            ...
            for objectDescription in OBJECTS:
                ...
                if not succeeded:
                    print(
                        'Failed to infer object "{}"'.format(objectDescription["name"])
                    )
...
if __name__ == "__main__":
    doPointsExperiment(cellDimensions=(10, 10), cellCoordinateOffsets=(0.05, 0.5, 0.95))
