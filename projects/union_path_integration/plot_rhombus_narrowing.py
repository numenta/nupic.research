...
def insertDiscreteModules(
    ...
    cellPhasesAxis = np.linspace(0.0, 1.0, locationModuleWidth, endpoint=False)
    cellPhases = np.array(
        [
            np.repeat(cellPhasesAxis, locationModuleWidth),
            np.tile(cellPhasesAxis, locationModuleWidth),
        ]
    ) + (0.5 / locationModuleWidth)
    ...
    for cell, (phi1, phi2) in enumerate(cellPhases.T):
        ...
        circle.set("cx", str(rhombusBase * (phi1 + phi2 * np.cos(np.radians(60.0)))))
        circle.set("cy", str(rhombusHeight * (1.0 - phi2)))
        circle.set("r", str(r))
...
def insertPointExcitations(
    ...
    bumpSigma,
    opacity=0.6,
    enhancementFactor=4.0,
    bumpOverlapMethod="probabilistic",
):
    imgWidth = rhombusBase * 1.5
    imgHeight = rhombusBase * np.sin(np.radians(60.0))

    numCols = int(rhombusBase * enhancementFactor)
    numRows = int(rhombusBase * np.sin(np.radians(60.0)) * enhancementFactor)
    ...
    queryPhases = np.array(
        [
            np.tile(np.linspace(0.0, 1.0, numCols, endpoint=False), numRows),
            np.repeat(np.linspace(0.0, 1.0, numRows, endpoint=False)[::-1], numCols),
        ]
    ) + [[0.5 / numCols], [0.5 / numRows]]
    ...
    columnOffsetByRow = np.floor(
        np.linspace(0, numCols * math.cos(np.radians(60.0)), numRows, endpoint=False)
    )[::-1].astype("int")
    columnInRhombus = columnOffsetByRow[:, np.newaxis] + np.arange(numCols)
    bitmap[(np.repeat(np.arange(numRows), numCols), columnInRhombus.flatten())] = (
        coloredSquare
    )
...
def rhombusChart(
    ...
    rhombusHeight = rhombusBase * np.sin(np.radians(60.0))

    with open(inFilename, "r") as f:
        experiments = json.load(f)

    exp = next(exp for exp in experiments if exp[0]["numObjects"] == 50)
    ...
    width = rhombusBase * (len(locationLayerTimeline) + 0.5) + betweenY * (
        len(locationLayerTimeline) - 1
    )
    height = rhombusHeight * len(moduleNumbers) + betweenY * (len(moduleNumbers) - 1)
    svg = ET.Element("svg")
    ...
    for t, moduleStates in enumerate(locationLayerTimeline):
        for iModule, moduleNumber in enumerate(moduleNumbers):
            ...
            insertDiscreteModules(
                rhombus,
                moduleState["activeCells"],
                locationModuleWidth,
                rhombusBase,
                rhombusHeight,
            )

            insertPointExcitations(
                rhombus,
                np.array(moduleState["bumps"]).T,
                rhombusBase,
                rhombusHeight,
                bumpSigma,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--moduleNumbers", type=int, nargs="+", default=list(range(0, 2, 9))
    )
    parser.add_argument("--numSteps", type=int, default=None)
    args = parser.parse_args()

    rhombusChart(
        args.inFile, args.outFile, args.objectNumber, args.moduleNumbers, args.numSteps
    )
