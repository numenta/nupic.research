...
def createTheChart(inFilename, outFilename, xlim2):
    ...
    for exp in experiments:
        ...
        capacityResults[(featureDistribution, numUniqueFeatures, featuresPerObject)][
            numObjects
        ].append(accuracy)
    ...
    for params, convergenceResultsByMin in convergenceResultsByParams.items():
        resultsByParams[params] = sorted(
            (
                sampleMinimum,
                float(sum(1 if r is not None else 0 for r in results)) / len(results),
            )
            for sampleMinimum, results in convergenceResultsByMin.items()
        )

    fig, (ax1, ax2) = plt.subplots(
        figsize=(6.75, 3), ncols=2, sharey=True, tight_layout={"pad": 0}
    )
    ...
    for i, (
        ...
        marker,
        markerSize,
    ) in enumerate(objectSets):
        if DETAILED_LABELS:
            if featureDistribution == "AllFeaturesEqual_Replacement":
                label = "{}, {} features, {} per object".format(
                    featureDistribution, numUniqueFeatures, featuresPerObject
                )
            else:
        ...
        resultsByNumObjects = capacityResults[
            (featureDistribution, numUniqueFeatures, featuresPerObject)
        ]
        expResults = sorted(
