...
from weight_init_experiment import WeightInitExperiment
...
def train_models(configs, project_dir):
    ...
    writer = SummaryWriter("./runs/basic")

    for exp in configs:
        config = configs[exp]
        if "name" not in config:
            config["name"] = exp
        print("Running exp %s" % config["name"])
        ...
        if config.get("checkpoint_at_end", False):
            exp.model_save(path)
            writer.export_scalars_to_json("./results/scalars.json")
