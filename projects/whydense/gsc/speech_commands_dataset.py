...
class PreprocessedSpeechDataset(Dataset):
    ...
    def __init__(self, root, subset, classes=CLASSES, silence_percentage=0.1):
        ...
        seeds = [int(e) for e in os.listdir(root) if e.isdigit()]
        random.shuffle(seeds)
        self._all_seeds = itertools.cycle(seeds)
    ...
    @staticmethod
    def is_valid(folder, seed=0):
        """Check if the given folder is a valid preprocessed dataset."""
        # Validate by checking for the training 'silence.pkl' on the given seed
        # This file is unique to our pre-processed dataset generated by
        # 'process_dataset.py'
        return os.path.exists(os.path.join(folder, str(seed), "train", "silence.pkl"))
