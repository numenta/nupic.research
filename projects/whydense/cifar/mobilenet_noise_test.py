...
from nupic.research.support import (
  parse_config, load_ray_tune_experiment)
...
class MobileNetNoiseTune(tune.Trainable):
  ...
  def _setup(self, config):
    ...
    from nupic.research.frameworks.pytorch.models import (
      MobileNetV1, MobileNetV1SparsePoint, MobileNetV1SparseDepth
    )

    # Cannot pass functions to "ray.tune". Make sure to use string in the config
    # and evaluate during "_setup"
    model_config = config["model_config"]
    model_config["loss_function"] = eval(model_config["loss_function"], globals(), locals())
    model_config["model_type"] = eval(model_config["model_type"], globals(), locals())
...
@click.option("--redis-address", help="Optional ray cluster redis address",
              show_envvar=True, envvar="REDIS_ADDRESS")
@click.option("-v", "--noise-values",  multiple=True, default=NOISE_VALUES,
              show_default=True, help="Noise values",  type=float)
