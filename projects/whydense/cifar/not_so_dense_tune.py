...
from projects.whydense.cifar.not_so_dense_experiment import NotSoDenseExperiment


class NotSoDenseTune(tune.Trainable):
    """ray.tune trainable class for running NotSoDense CIFAR experiments:"""
    ...
    def _setup(self, config):
        ...
        self.experiment = NotSoDenseExperiment(config)
    ...
    for exp in configs:
        ...
        results.append(
            run_experiment.remote(
                config, NotSoDenseTune, num_cpus=num_cpus, num_gpus=num_gpus / num_cpus
            )
        )
