...
import convergence_activity_paper
...
def generateFigure3(cpuCount):
    ...
    """
    convergence_activity_paper.runExperiment()
...
def generateFigure5A(cpuCount):
    ...
    """
    capacity_test.runExperiment3(
        cpuCount=cpuCount, resultDirName=RESULT_DIR_NAME, plotDirName=PLOTS_DIR_NAME
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=("Use this script to generate the figures and results "
                     "presented in (Hawkins et al., 2017)"),
        epilog="""
        -----------------------------------------------------------------------------
        Jeff Hawkins, Subutai Ahmad, Yuwei Cui (2017),
        A Theory of How Columns in the Neocortex Enable Learning the Structure of the
    ...
    parser.add_argument("-l", "--list", action="store_true", help="List all figures")
    ...
    if opts.list:
        ...
        generateFigureFunc[opts.figure](cpuCount=opts.cpuCount)
    else:
        parser.print_help()
