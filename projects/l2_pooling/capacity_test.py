...
def _prepareResultsDir(resultBaseName, resultDirName=DEFAULT_RESULT_DIR_NAME):
    ...
    try:
        ...
        if not os.path.isdir(resultDirName):
            raise Exception(
                "Unable to create results directory at {}".format(resultDirName)
            )
...
def createRandomObjects(numObjects, numPointsPerObject, numLocations, numFeatures):
    """
    ...
    randomPairIdx = np.random.choice(
        np.arange(uniqueFeatureLocPairs), numObjects * numPointsPerObject, replace=False
    )
...
def createRandomObjectsSharedPairs(
    ...
    for _ in range(numObjects):
        objectLocations = np.random.choice(locations, numPointsPerObject, replace=False)
        objectFeatures = np.random.choice(features, numPointsPerObject, replace=True)
...
def testNetworkWithOneObject(objects, exp, testObject, numTestPoints):
    ...
    testPts = np.random.choice(
        np.arange(numPointsPerObject), (numTestPoints,), replace=False
    )
    ...
    for step, pair in enumerate(testPairs):
        ...
        sensations = {
            colIdx: (
                objects.locations[colIdx][locationIdx],
        ...
        # print "{} L4 cells active : ".format(len(exp.getL4Representations()[0])), exp.getL4Representations()  # noqa
...
def testOnSingleRandomSDR(objects, exp, numRepeats=100, repeatID=0):
    ...
    for i in range(numRepeats):
        ...
        overlap, numActiveL2Cells, numL4ActiveCells = testNetworkWithOneObject(
            objects, exp, targetObject, 10
        )

        lastOverlap = overlap[-1, :]

        maxOverlapIndices = np.where(
            lastOverlap == lastOverlap[np.argmax(lastOverlap)]
        )[0].tolist()
        ...
        result = (
            ...
            else pd.DataFrame.from_dict([testResult])
        )
...
def plotResults(
    ...
    numberOfConnectedProximalSynapses = np.zeros(
        (
            1,
            len(x),
        )
    )
    l2ActivationSize = np.zeros(
        (
            1,
            len(x),
        )
    ...
    if showErrBar == 0:
        ...
        s = 1
    ax[0, 0].errorbar(
        x, np.mean(accuracy, 0), yerr=np.std(accuracy, 0) * s, color=marker
    )
    ...
    ax[0, 1].errorbar(
        x,
        np.mean(numberOfConnectedProximalSynapses, 0),
    ...
    ax[1, 0].errorbar(
        x,
        np.mean(l2ActivationSize, 0),
        yerr=np.std(l2ActivationSize, 0) * s,
        color=marker,
    )
    ...
    ax[1, 1].errorbar(
        x, np.mean(confusion, 0), yerr=np.std(confusion, 0) * s, color=marker
    )
...
def runCapacityTestVaryingObjectSize(
    ...
    for testResult in pool.map(invokeRunCapacityTest, params):
        result = pd.concat([result, testResult]) if result is not None else testResult
...
def runCapacityTestVaryingObjectNum(
...
def runCapacityTestWrapperNonParallel(
    numPointsPerObject=10,
    numCorticalColumns=DEFAULT_NUM_CORTICAL_COLUMNS,
    resultDirName=DEFAULT_RESULT_DIR_NAME,
    numObjects=100,
...
def runExperiment1(
    ...
    l4Params["activationThreshold"] = int(numInputBits * 0.6)
    l4Params["minThreshold"] = int(numInputBits * 0.6)
    ...
    for sampleSize in sampleSizeRange:
        ...
        runCapacityTestVaryingObjectSize(
            numObjects,
            numCorticalColumns,
            resultDirName,
            expName,
    ...
    fig, ax = plt.subplots(2, 2)
    st = fig.suptitle(
        "Varying points per object x 2 objects ({} cortical column{})".format(
    ...
    plt.savefig(os.path.join(plotDirName, "capacity_varying_object_size_summary.pdf"))
...
def runExperiment2(
    ...
    plt.savefig(os.path.join(plotDirName, "capacity_varying_object_num_summary.pdf"))
...
def runExperiment3(
    ...
    expParams = []
    expParams.append(
        {"l4Column": 150, "externalInputSize": 2400, "w": 20, "sample": 10, "thresh": 5}
    )
    expParams.append(
        {"l4Column": 200, "externalInputSize": 2400, "w": 20, "sample": 10, "thresh": 5}
    )
    expParams.append(
        {"l4Column": 250, "externalInputSize": 2400, "w": 20, "sample": 10, "thresh": 5}
    )
...
def runExperiment6(
    ...
    for expParam in expParams:
        ...
        expName = "multiple_column_capacity_varying_object_sdrSize_{}_l2Cells_{}_l2column_{}".format(  # noqa
            expParam["L2activeBits"], expParam["L2cellCount"], expParam["l2Column"]
        )
    ...
    plt.savefig(os.path.join(plotDirName, "capacity_vs_L2_sparsity.pdf"))
...
def runExperiment7(
    ...
    for expParam in expParams:
        ...
        expname = "multiple_column_capacity_varying_object_num_locations_{}_num_features_{}_l4column_{}".format(  # noqa
    ...
    fig, ax = plt.subplots(2, 2)
    st = fig.suptitle(
        "Varying number of objects ({} cortical column{})".format(  # noqa
            numCorticalColumns, "s" if numCorticalColumns > 1 else ""
        ),
        fontsize="x-large",
    )
    ...
    plt.savefig(
        os.path.join(
            plotDirName, "capacity_varying_object_num_locations_num_summary.pdf"
        )
    )
...
def runExperiment8(
    ...
    plt.savefig(
        os.path.join(
            plotDirName, "capacity_varying_object_num_features_num_summary.pdf"
        )
    )
...
def runExperiment9(
    ...
    for object_num in reversed(objectNumRange):
        for expParam in expParams:
            for rpt in range(numRpts):
                ...
                expName = "multiple_column_capacity_varying_object_num_synapses_{}_thresh_{}_l4column_{}_l2column_{}_{}".format(  # noqa
                    expParam["sample"],
                    expParam["thresh"],
                    expParam["l4Column"],
                    expParam["l2Column"],
    ...
    pool = multiprocessing.Pool(
        cpuCount or multiprocessing.cpu_count(), maxtasksperchild=1
    )
    ...
    colors = [colormap(x) for x in np.linspace(0.0, 1.0, len(expParam))]
    ...
    plt.savefig(
        os.path.join(
            plotDirName,
            "multiple_column_capacity_varying_object_num_column_num_connection_type_summary.pdf",  # noqa
        )
    )
...
def runExperiment10(
    ...
    for expParam in expParams:
        ...
        l2Params["cellCount"] = expParam["L2cellCount"]
        l2Params["sdrSize"] = expParam["L2activeBits"]
        l2Params["sampleSizeDistal"] = int(expParam["L2cellCount"] * 0.5)
        l2Params["activationThresholdDistal"] = int(expParam["L2cellCount"] * 0.5) - 1
        ...
        expname = "multiple_column_capacity_varying_object_num_synapses_{}_thresh_{}_l4column_{}_l2cell_{}".format(  # noqa
            expParam["sample"],
            expParam["thresh"],
            expParam["l4Column"],
            expParam["L2cellCount"],
    ...
    plt.savefig(
        os.path.join(plotDirName, "capacity_varying_object_num_l4l2size_summary.pdf")
    )
...
def runExperiments(resultDirName, plotDirName, cpuCount):
    ...
    runExperiment4(
        resultDirName=resultDirName, plotDirName=plotDirName, cpuCount=cpuCount
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--plotDirName", default=DEFAULT_PLOT_DIR_NAME, type=str, metavar="DIRECTORY"
    )
    parser.add_argument(
        "--cpuCount",
        default=None,
