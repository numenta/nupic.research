...
def plotConvergenceByColumnTopology(
    results, columnRange, featureRange, networkType, numTrials
):
    """
    ...
    for r in results:
        convergence[
            r["numFeatures"] - 1, r["numColumns"], networkType.index(r["networkType"])
        ] += r["convergencePoint"]
    ...
    colorList = [
        colormap(x)
        for x in numpy.linspace(0.0, 1.0, len(featureRange) * len(networkType))
    ]
...
def plotConvergenceByDistantConnectionChance(
    results, featureRange, columnRange, longDistanceConnectionsRange, numTrials
):
    """
    Plots the convergence graph: iterations vs number of columns.
    Each curve shows the convergence for a given number of unique features.
    ...
    for r in results:
        ...
        convergence[
            featureRange.index(r["numFeatures"]),
            longDistanceConnectionsRange.index(r["longDistanceConnections"]),
            columnRange.index(r["numColumns"]),
        ] += r["convergencePoint"]
    ...
    colorList = [
        ...
        for x in numpy.linspace(
            0.0, 1.0, len(featureRange) * len(longDistanceConnectionsRange)
        )
    ]
...
def main():
    ...
    if False:
        ...
        plotConvergenceByColumnTopology(
            results, columnRange, featureRange, networkType, numTrials=numTrials
        )
    ...
    if False:
        ...
        runExperimentPool(
            ...
            nTrials=numTrials,
            numWorkers=cpu_count() - 1,
            resultsName="random_long_distance_connection_column_convergence_results.pkl",  # noqa
        )

        with open(
            "random_long_distance_connection_column_convergence_results.pkl", "rb"
        ) as f:
            results = pickle.load(f)
