    ...
    fig["layout"]["annotations"].append(
        {
            ...
            "text": [
                "",
                "<b>One cortical column</b>",
                "",
                "<b>Three cortical columns</b>",
            ][numColumns],
            "xref": "paper",
            "yref": "paper",
            "x": 0.5,
...
def plotL2ObjectRepresentations(exp1):
    ...
    plotPath = plotly.offline.plot(
        fig, filename="plots/shapes-rectangle.html", auto_open=True
    )
    print("url=", plotPath)
...
def main():
    ...
    objectMachine.createRandomObjects(
        numObjects,
        numPoints=numPoints,
    ...
    for step in range(len(objectSensations[0])):
        pairs = [objectSensations[col][step] for col in range(numColumns)]
