...
class CorticalColumn(object):
    ...
    def getAllCellActivity(self):
        return (
            ...
            tuple(
                set(module.activeCells) for module in self.sensorToSpecificObjectModules
            ),
        )
...
class MultiColumn2DExperiment(object):
    ...
    def __init__(
        ...
        self.features = dict(
            (
                (iCol, k),
                np.array(sorted(random.sample(range(150), featureW)), dtype="uint32"),
            )
        ...
        for _ in range(numCorticalColumns):
            inputLayer = ApicalTiebreakPairMemory(
                **{
                    ...
                    "basalInputSize": sum(
                        np.prod(config["cellDimensions"]) for config in locationConfigs
                    ),
            ...
            sensorToBodyModules = [
                SensorToBodyModule2D(**config) for config in locationConfigs
            ]
        ...
        for iModule, config in enumerate(locationConfigs):
            ...
            pairedSensorModules = [
                c.sensorToSpecificObjectModules[iModule] for c in self.corticalColumns
            ]
    ...
    def compute(self, egocentricLocationByColumn, featureSDRByColumn, learn):
        ...
        paramsByModuleByColumn = [
            [{"egocentricLocation": egocentricLocation} for _ in c.sensorToBodyModules]
            for c, egocentricLocation in zip(
                self.corticalColumns, egocentricLocationByColumn
            )
        ]
        ...
        paramsByModuleByColumn = [
            [
                ...
                for sensorToBody, sensorToSpecificObject, bodyToSpecificObject in zip(
                    c.sensorToBodyModules,
        ...
        for c, paramsByModule in zip(self.corticalColumns, paramsByModuleByColumn):
            for module, params in zip(c.sensorToSpecificObjectModules, paramsByModule):
        ...
        # Layer 6A
        paramsByColumn = [
            {"anchorInput": c.inputLayer.getActiveCells(), "learn": learn}
            for c in self.corticalColumns
        ]
        ...
        for module, params in zip(self.bodyToSpecificObjectModules, paramsByModule):
            module.compute(**params)
    ...
    def learnObjects(self, bodyPlacement):
        ...
        for objectName, objectFeatures in self.objects.items():
            ...
            for iFeatureStart in range(len(objectFeatures)):
                featureIndexByColumn = np.mod(
                    np.arange(iFeatureStart, iFeatureStart + self.numCorticalColumns),
                    len(objectFeatures),
                )

                featureByColumn = [
                    objectFeatures[iFeature] for iFeature in featureIndexByColumn
                ]
                featureSDRByColumn = [
                    self.features[(iCol, feature["name"])]
                ...
                for t in range(2):
                    for monitor in self.monitors.values():
                        monitor.beforeCompute(
                            egocentricLocationByColumn,
                            featureSDRByColumn,
                            isRepeat=(t > 0),
                        )
                    self.compute(
                        egocentricLocationByColumn, featureSDRByColumn, learn=True
                    )

                for iFeature, locationOnObject, c in zip(
                    featureIndexByColumn, locationOnObjectByColumn, self.corticalColumns
                ):
                    ...
                    c.inputRepresentations[
                        (objectName, locationOnObject, objectFeatures[iFeature]["name"])
    ...
    def inferObjects(self, bodyPlacement, maxTouches=2):
        ...
        for objectName, objectFeatures in self.objects.items():
            ...
            featureIndexByColumnIterator = greedySensorPositions(
                self.numCorticalColumns, len(objectFeatures)
            )
            ...
            for touch in range(maxTouches):
                ...
                for t in range(self.maxSettlingTime):
                    ...
                    self.compute(
                        egocentricLocationByColumn, featureSDRByColumn, learn=False
                    )

                    cellActivity = (
                        tuple(c.getAllCellActivity() for c in self.corticalColumns),
                        tuple(
                            set(module.activeCells)
                            for module in self.bodyToSpecificObjectModules
                        ),
                    )
...
class MultiColumn2DExperimentMonitor(object, metaclass=abc.ABCMeta):
    ...
    def beforeCompute(self, egocentricLocationByColumn, featureSDRByColumn, isRepeat):
        pass
    ...
    def afterSensorMetricCompute(self, paramsByColumn):
        pass

    def afterSensorLocationAnchor(self, paramsByColumn):
        pass

    def afterBodyLocationAnchor(self, params):
        pass
