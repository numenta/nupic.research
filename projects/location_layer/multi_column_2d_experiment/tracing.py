...
class MultiColumn2DExperimentVisualizer(MultiColumn2DExperimentMonitor):
    ...
    def beforeCompute(self, egocentricLocationByColumn, featureSDRByColumn, isRepeat):
        ...
        print(
            json.dumps([featureSDR.tolist() for featureSDR in featureSDRByColumn]),
            file=self.out,
        )
        print(
            json.dumps(
                [
                    [
                        ...
                        if iCol == iCol2
                        if np.intersect1d(featureSDR, sdr, assume_unique=True).size
                        == sdr.size
                    ]
        ...
        self.prevActiveObjectCellsByColumn = [
            c.objectLayer.getActiveCells() for c in self.exp.corticalColumns
        ]
    ...
    def afterSensorToBodyCompute(self, paramsByColumn):
        cellsByModuleByColumn = [
            [module.getActiveCells().tolist() for module in c.sensorToBodyModules]
    ...
    def afterSensorMetricCompute(self, paramsByModuleByColumn):
                    ...
                    cellsByModule.append(
                        [
                            activeCells.tolist(),
                            {
                                ...
                                "bodyToSpecificObject": _getActiveSynapsesOnActiveSegments(
                                    module.metricConnections.connectionsBySource[
        ...
        for c in self.exp.corticalColumns:
            ...
            for (objectName, location), sdr in c.sensorLocationRepresentations.items():
                amountContained = np.intersect1d(
                    sdr, activeLocationCells, assume_unique=True
                ).size / float(sdr.size)
                decodings.append(
    ...
    def afterBodyLocationAnchor(self, paramsByModule):
        ...
        for iModule, module in enumerate(self.exp.bodyToSpecificObjectModules):
            ...
            if self.includeSynapses:
                ...
                for iPresynapticCol, metricConnections in enumerate(
                    module.connectionsByColumn
                ):
                    synapsesForActiveCellsBySourceLayer[
        ...
        for iCol, c in enumerate(self.exp.corticalColumns):
            ...
            if self.includeSynapses:
                segmentsForCells = {
                    ...
                    "{} objectLayer".format(iCol): _getActiveSynapsesOnActiveSegments(
                ...
                cellsByColumn.append([cells.tolist(), segmentsForCells])
        ...
        for iCol, c in enumerate(self.exp.corticalColumns):
            ...
            if self.includeSynapses:
                ...
                for cell in activeCells:
                    inputSynapsesForActiveCells.append(
                        _getActiveSynapses(
                            c.objectLayer.proximalPermanences,
                            cell,
                            params["feedforwardInput"],
                    ...
                    for iPresynapticCol in range(self.exp.numCorticalColumns):
                        if iPresynapticCol == iCol:
                            ...
                            synapsesOnSegment = _getActiveSynapses(
                                ...
                                cell,
                                params["lateralInputs"][matrixIndex],
                                c.objectLayer.connectedPermanenceDistal,
                            )

                        objectSynapsesForActiveCellsBySourceColumn[
                            iPresynapticCol
                        ].append(synapsesOnSegment)
                ...
                synapsesForActiveCellsBySourceLayer["{} inputLayer".format(iCol)] = (
                    inputSynapsesForActiveCells
                )
            ...
            decodingsByColumn.append(
                [
                    ...
                    if np.intersect1d(activeCells, sdr, assume_unique=True).size
                    == sdr.size
                ]
            )
...
def _getActiveSynapsesOnActiveSegments(
    connections, cells, activeSegments, activeInput, connectedPermanence, offset=0
):
    ...
    for i, segment in enumerate(segments):
        connectedSynapses = np.where(
            connections.matrix.getRow(segment) >= connectedPermanence
        )[0]

        activeSynapses = np.intersect1d(
            connectedSynapses, activeInput, assume_unique=True
        )
        if offset != 0:
    ...
    return [sorted(set(synapsesForCellDict[cell])) for cell in cells]
...
def _getActiveSynapses(matrix, cell, activeInput, connectedPermanence):
    ...
    activeSynapses = np.intersect1d(connectedSynapses, activeInput, assume_unique=True)
    return activeSynapses.tolist()


def _getInputDecodings(c, activeCells):
    decodings = []
    for (objectName, location, _feature), sdr in c.inputRepresentations.items():
        amountContained = np.intersect1d(
            sdr, activeCells, assume_unique=True
        ).size / float(sdr.size)
