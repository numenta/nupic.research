...
from generate_sdrs import (
    carefullyCollideContexts,
    createEvenlySpreadSDRs,
    generateMinicolumnSDRs,
)
from nupic.bindings.math import SparseMatrix, SparseMatrixConnections
...
class ForwardModel(object):
    ...
    def associate(self, activeCells, activeInput):
        ...
        self.permanences.setZerosOnOuter(self.activeCells, activeInput, 1.0)
...
class SegmentedForwardModel(object):
    ...
    def associate(self, activeCells, activeInput):
        self.activeCells = activeCells
        self.activeSegments = self.proximalConnections.createSegments(activeCells)
...
class PoolOfPairsLocation1DExperiment(object):
    ...
    def trainWithSpecificPairSDRs(self, pairLayerContexts):
        ...
        for iDriving, cDriving in enumerate(self.drivingOperandSDRs):
            ...
            for iContext, _cContext in enumerate(self.contextOperandSDRs):
                iResult = (iContext + iDriving) % self.numLocations
                cResult = self.resultSDRs[iResult]
                cPair = [
    ...
    def testInferenceOnUnions(self, unionSize, numTests=300):
        ...
        for _ in range(numTests):
            iContexts = random.sample(range(self.numLocations), unionSize)
            iDriving = random.choice(range(self.numLocations))
            cDriving = self.drivingOperandSDRs[iDriving]

            cContext = np.unique(
                np.concatenate(
                    [self.contextOperandSDRs[iContext] for iContext in iContexts]
                )
            )
            cResultExpected = np.unique(
                np.concatenate(
                    [
            ...
            additionalSDRCounts.append(
                np.setdiff1d(self.poolingLayer.activeCells, cResultExpected).size
                / self.numActiveResultCells
            )
...
def runExperiment(
    n, w, threshold, cellsPerColumn, folder, numTrials=5, cleverTMSDRs=False
):
    """
    ...
    if len(glob.glob(filename)) == 0:
        print(
            "Starting: {}/n_{}_w_{}_threshold_{}_cellsPerColumn_{}".format(
                folder, n, w, threshold, cellsPerColumn
            )
        )
        ...
        for _ in range(numTrials):
            ...
            exp = PoolOfPairsLocation1DExperiment(
                **{
                    ...
                    "cellsPerColumn": cellsPerColumn,
                    "minicolumnSDRs": generateMinicolumnSDRs(
                        n=n, w=w, threshold=threshold
                    ),
                }
            )

            if cleverTMSDRs:
                exp.trainWithSpecificPairSDRs(
                    carefullyCollideContexts(
        ...
        with open(filename, "w") as fOut:
            json.dump(sorted(list(result.items()), key=lambda x: x[0]), fOut)
...
def run(allParams, parallel=True):
    if parallel:
        ...
        multiprocessing.Pool().map(
            doGenerateMinicolumnSDRs,
            set(
                (params["n"], params["w"], params["threshold"]) for params in allParams
            ),
        )
