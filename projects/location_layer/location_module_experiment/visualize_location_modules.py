...
def go():
    ...
    with open(filename, "w") as fileOut:
        ...
        print(json.dumps({"width": worldWidth, "height": worldHeight}), file=fileOut)
        print(json.dumps(loggedLocationConfigs), file=fileOut)

        modules = [
            Superficial2DLocationModule(anchorInputSize=0, **config)
        ...
        for deltaLocation in 30 * [[2.0, 2.5]]:
            ...
            cellsByModule = [module.getActiveCells().tolist() for module in modules]
            print(json.dumps(cellsByModule), file=fileOut)
            pointsByModule = []
            for module in modules:
                pointsByModule.append(
                    (module.activePhases * module.cellDimensions).tolist()
                )
...
if __name__ == "__main__":
    ...
    print(
        "http://numenta.github.io/htmresearch/visualizations/location-layer/location-modules.html"
    )
    print(
