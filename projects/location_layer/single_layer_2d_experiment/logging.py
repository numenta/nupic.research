...
class SingleLayer2DExperimentVisualizer(SingleLayer2DExperimentMonitor):
    ...
    def afterLocationCompute(
        ...
        for i, segment in enumerate(deltaSegments):
            connectedDeltaSynapses = np.where(
                self.locationLayer.deltaConnections.matrix.getRow(segment)
                >= self.locationLayer.connectedPermanence
            )[0]
            connectedInternalSynapses = np.where(
                self.locationLayer.internalConnections.matrix.getRow(segment)
                >= self.locationLayer.connectedPermanence
            )[0]

            activeDeltaSynapses = np.intersect1d(connectedDeltaSynapses, deltaLocation)
            activeInternalSynapses = np.intersect1d(
                connectedInternalSynapses, self.prevActiveLocationCells
            )
            segmentData = [
        ...
        for i, segment in enumerate(featureLocationSegments):
            ...
            segmentData = [["input", activeFeatureLocationSynapses.tolist()]]
    ...
    def afterInputCompute(
        ...
        for i, segment in enumerate(apicalSegments):
            ...
            segmentData = [["object", activeSynapses.tolist()]]
    ...
    def afterObjectCompute(
        ...
        for cell in activeCells:
            ...
            segmentData = [["input", activeSynapses.tolist()]]
            cells[cell].append(segmentData)
