...
def add_sparse_cnn_layer(
    ...
    if 0 < weight_sparsity < 1.0:
        ...
        network.add_module(cnn_string + "{}_cnn".format(suffix), sparse_cnn)
    else:
        network.add_module(cnn_string + "{}_cnn".format(suffix), cnn)

    if use_batch_norm:
        bn = nn.BatchNorm2d(out_channels, affine=False)
        network.add_module(cnn_string + "{}_bn".format(suffix), bn)

    if not activation_fct_before_max_pool:
        maxpool = nn.MaxPool2d(kernel_size=2)
        network.add_module(cnn_string + "{}_maxpool".format(suffix), maxpool)

    if percent_on >= 1.0 or percent_on <= 0:
        network.add_module(cnn_string + "{}_relu".format(suffix), nn.ReLU())
    else:
        ...
        network.add_module(cnn_string + "{}_kwinner".format(suffix), kwinner)
...
class PredSparseNet(nn.Sequential):
    """
    ...
    def __init__(self,
                 ...
                 cnn_trans_channels=(64, 64),
                 cnn_trans_percent_on=(0.1, 0.1),
                 cnn_trans_weight_percent_on=(0.1, 0.1),
                 trans_kernel_sizes=(5, 8),
                 trans_padding=(0,1),
                 trans_stride=(1,1),
                 linear_n=(1000,),
        ...
        super(PredSparseNet, self).__init__()
...
#         self.add_module("flatten", Flatten())
        for i in range(len(cnn_trans_channels)):
            in_channels, height, width = current_input_shape
            ...
            add_sparse_cnn_layer(
                ...
                suffix=i + 1,
                in_channels=in_channels,
                out_channels=cnn_trans_channels[i],
                kernel_size=trans_kernel_sizes[i],
                padding=trans_padding[i],
                stride=trans_stride[i],
                use_batch_norm=use_batch_norm,
                weight_sparsity=cnn_trans_weight_percent_on[i],
                percent_on=cnn_trans_percent_on[i],
                k_inference_factor=k_inference_factor,
                boost_strength=boost_strength,
                boost_strength_factor=boost_strength_factor,
                duty_cycle_period=duty_cycle_period,
            ...
            wout = (width - 1) - 2 * trans_padding[i] + trans_kernel_sizes[i] + trans_padding[i]
            maxpool_width = wout // 2
            current_input_shape = (cnn_trans_channels[i], maxpool_width, maxpool_width)
            print(current_input_shape)
