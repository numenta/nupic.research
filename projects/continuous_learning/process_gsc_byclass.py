...
data_dir = "/Users/afisher/nta/nupic.torch/examples/gsc/data/"
...
for k in range(1,12):
    ...
    with open("data_classes/data_train_{}.npz".format(k), 'wb') as f:
...
for k in range(1,12):
    ...
    labels_tensor = torch.Tensor(data_tensor.shape[0]*[k]).long()
    out_tensor = list((data_tensor, labels_tensor))
    with open("data_classes/data_valid.npz", 'wb') as f:
        torch.save(out_tensor, f)
...
for k in range(1,12):
    ...
    labels_tensor = torch.Tensor(data_tensor.shape[0]*[k]).long()
    out_tensor = list((data_tensor, labels_tensor))
    with open("data_classes/data_test_{}.npz".format(k), 'wb') as f:
        torch.save(out_tensor, f)
