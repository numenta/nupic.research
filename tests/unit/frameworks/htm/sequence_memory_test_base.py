...
class SequenceMemoryTestBase(object, metaclass=ABCMeta):
    ...
    should be the same as B1. Since num_cells_per_minicolumn == 1, it should make more
    predictions than necessary.
    H2) Same as H1, but with num_cells_per_minicolumn == 32, and train multiple times.
...
class ApicalTiebreakTM_SequenceMemoryTests(SequenceMemoryTestBase, unittest.TestCase):
    ...
    def compute(self, active_minicolumns, learn):
        active_minicolumns = torch.Tensor(list(sorted(active_minicolumns))).to(int_type)

        self.tm.compute(active_minicolumns, learn=learn)
