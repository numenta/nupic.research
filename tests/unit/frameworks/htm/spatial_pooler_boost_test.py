...
class SpatialPoolerBoostTest(unittest.TestCase):
    ...
    def set_up(self):
        ...
        # for each minicolmn, this will contain the last iteration number where that
        # column was a winner
        self.winning_iteration = np.zeros(self.minicolumn_dims)

        # for each input vector i, last_sdr[i] contains the most recent SDR output by
        # the SP
        self.last_sdr = {}
        ...
        self.sp = SpatialPooler(
            ...
            active_minicolumns_per_inh_area=60,
            local_density=-1,
    ...
    def verify_sdr_properties(self):
        ...
        the bounds of checking overlap are loosely set since there is some variance due
        to randomness and the artificial parameters used in this test.
        """
    ...
    def boost_test_phase2(self):
        ...
        # every minicolumn should have been sufficiently boosted to win at least once.
        # number of minicolumns that have never won should be 0
        num_losers_after = (self.winning_iteration == 0).sum()
        self.assertEqual(num_losers_after, 0)

        # artificially induced thrashing --> even the first two patterns should have low
        # overlap. verify this.
        self.assertLess(
            compute_overlap(self.last_sdr[0], self.last_sdr[1]),
            7,
    ...
    def boost_test_phase3(self):
        ...
        # by now, every minicolumn should have been sufficiently boosted to win at least
        # once. number of minicolumns that have never won should be 0.
        num_losers_after = (self.winning_iteration == 0).sum()
        self.assertEqual(num_losers_after, 0)
