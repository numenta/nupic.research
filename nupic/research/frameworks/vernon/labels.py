...
def get_label_mappings():
    """
    Return human readable ImageNet labels.
    :return:
        (index_to_label, class_to_label)
        index_to_label is a list such that index_to_label[i] is the human readable
            label for class index i. For example, index_to_label[3] is "tiger_shark"
        class_to_label is a dict that maps class names (such as "n01443537") to human
            readable labels, such as "goldfish"
    """
    index_to_label = [IMAGENET_LABELS[str(k)][1] for k in range(len(IMAGENET_LABELS))]
    class_to_label = {IMAGENET_LABELS[str(k)][0]: IMAGENET_LABELS[str(k)][1] for k in
                      range(len(IMAGENET_LABELS))}

    return index_to_label, class_to_label
