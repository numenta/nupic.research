...
def percent_active_dendrites(dendrite_segments, context_vectors, selection_criterion):
    ...
    for j in range(len(context_vectors)):
        ...
        percentage_activations = torch.cat((percentage_activations, selected), dim=2)
...
def dendrite_overlap_matrix(dendrite_segments, context_vectors, selection_criterion):
    ...
    # num_categories); for each unit's dendrites, compute the dot product between all
    # pairs of columns; the resulting tensor will have shape (num_units,
