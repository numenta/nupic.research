...
class L4L2ExperimentTest(unittest.TestCase):
    ...
    def testSimpleExperiment(self):
        ...
        self.assertSequenceEqual(
            stats[0]["Overlap L2 with object C0"], [40, 40, 40, 40]
        )
        self.assertSequenceEqual(
            stats[0]["Overlap L2 with object C1"], [40, 40, 40, 40]
        )
    ...
    def testDelayedLateralandApicalInputs(self):
        ...
        def record_l2(prev_inputs, l2_column):
            ...
            def l2_compute_wrapped(
                feedforwardInput, feedforwardGrowthCandidates, lateralInputs, learn
            ):
                prev_inputs.clear()
                prev_inputs.update(
                    {
        ...
        def record_l4(prev_inputs, l4_column):
            ...
            def l4_compute_wrapped(activeColumns, basalInput, apicalInput, learn):
                prev_inputs.clear()
                prev_inputs.update(
                    {"basalInput": basalInput, "apicalInput": apicalInput}
                )
                return l4_compute(
        ...
        for step in range(3):
            ...
            exp.infer(
                objects.provideObjectToInfer(inferConfig), objectName=1, reset=False
            )
        ...
        for step in range(2):
            numpy.testing.assert_equal(
                lateralInputs[step + 1][0][0], activeCells[step][1]
            )
            numpy.testing.assert_equal(
                lateralInputs[step + 1][1][0], activeCells[step][0]
            )

        # apical inputs of L4_0 at time t+1 = active cells of L2_0 at time t
        for step in range(2):
            numpy.testing.assert_equal(apicalInputs[step + 1][0], activeCells[step][0])
            numpy.testing.assert_equal(apicalInputs[step + 1][1], activeCells[step][1])
    ...
    def testCapacity(self):
        ...
        self.assertEqual(stats[0]["object"], "obj1")
        self.assertSequenceEqual(
            stats[0]["Overlap L2 with object C0"], [10, 10, 10, 10]
        )
        self.assertSequenceEqual(
            stats[0]["Overlap L2 with object C1"], [10, 10, 10, 10]
        )
    ...
    def testObjectClassificationUnit(self):
        ...
        with patch.object(
            l2_l4_inference.L4L2Experiment, "getL2Representations"
        ) as mock_getL2Representations:

            exp = l2_l4_inference.L4L2Experiment(
                "testClassificationUnit", numCorticalColumns=5
            )
    ...
    def testObjectClassification(self):
        ...
        # Try to infer "Mug" using first learned grasp
        sensations = [objectsToLearn["Mug"][0], objectsToLearn["Mug"][1]]
        exp.sendReset()
        ...
        sensations = [objectsToLearn["Can"][0], objectsToLearn["Can"][1]]
        exp.sendReset()
        exp.infer(sensations, reset=False)
        results = exp.getCurrentClassification()
        ...
        sensations = [objectsToLearn["Box"][0], objectsToLearn["Box"][1]]
        exp.sendReset()
        exp.infer(sensations, reset=False)
        results = exp.getCurrentClassification()
        self.assertEqual(results["Mug"], 0)
