...
class ColumnPoolerTest(unittest.TestCase):
    ...
    def testInitialNullInputLearnMode(self):
        ...
        self.assertEqual(
            len(pooler.getActiveCells()), 0, "Incorrect number of active cells"
        )
        ...
        self.assertEqual(len(objectSDR2), 40, "Incorrect number of active cells")
        self.assertLess(
            len(objectSDR1 & objectSDR2), 5, "SDRs not sufficiently different"
        )

    def testInitialProximalLearning(self):
        """Tests the first few steps of proximal learning."""

        pooler = MonitoredColumnPooler(inputWidth=2048 * 8, cellCount=2048)

        # Get initial activity
        pooler.compute(feedforwardInput=list(range(0, 40)), learn=True)
        self.assertEqual(
            len(pooler.getActiveCells()), 40, "Incorrect number of active cells"
        )
    ...
    def testProximalLearning_SampleSize(self):
        ...
        for cell in pooler.getActiveCells():
            self.assertEqual(
                pooler.numberOfProximalSynapses([cell]),
                10,
                "Should connect to every active input bit.",
            )
            self.assertEqual(
                pooler.numberOfConnectedProximalSynapses([cell]),
                10,
                "Each synapse should be marked as connected.",
            )

            (presynapticCells, permanences) = pooler.proximalPermanences.rowNonZeros(
                cell
            )

            self.assertEqual(
                set(presynapticCells),
                set(feedforwardInput1),
                "Should connect to every active input bit.",
            )
        ...
        for cell in activeCells:
            ...
            for presynapticCell in range(0, 15):
                ...
                self.assertAlmostEqual(
                    ...
                    0.55,
                    msg=(
                        "Should have permanence of 'initialProximalPermanence'"
                        " + 'synPermProximalInc'."
                    ),
                )
    ...
    def testProximalLearning_PunishExisting(self):
        ...
        for cell in activeCells:
            ...
            self.assertEqual(
                pooler.numberOfConnectedProximalSynapses([cell]),
                5,
                "Each punished synapse should no longer be marked as" " connected.",
            )
    ...
    def testLearningWithLateralInputs(self):
        ...
        self.assertEqual(
            pooler.numberOfDistalSegments(activeCells),
            40,
            "Incorrect number of segments after learning",
        )
        self.assertEqual(
            pooler.numberOfDistalSynapses(activeCells),
            40 * 20,
        ...
        self.assertEqual(
            pooler.numberOfDistalSegments(list(range(2048))),
            80,
        ...
        self.assertEqual(
            pooler.numberOfDistalSegments(uniqueCellsObject2),
            len(uniqueCellsObject2),
            "Incorrect number of segments after learning",
        )
        self.assertEqual(
            pooler.numberOfDistalSynapses(uniqueCellsObject2),
    ...
    def testInferenceWithLateralInputs(self):
        ...
        for obj in range(2):
            ...
            for _ in range(3):  # three iterations
                for f in range(3):  # three features per object
                    pooler.compute(
                        feedforwardInputs[obj][f], objectLateralInputs[obj], learn=True
                    )
        ...
        pooler.reset()
        pooler.compute(
            feedforwardInput=list(
                set(feedforwardInputs[0][0]) | set(feedforwardInputs[1][1])
            ),
            learn=False,
        )
        self.assertEqual(
        ...
        pooler.reset()
        pooler.compute(
            feedforwardInput=feedforwardInputs[0][0],
        ...
        pooler.compute(
            feedforwardInput=(), lateralInputs=objectLateralInputs[0], learn=False
        )
        ...
        for obj in range(2):
            ...
            for col in range(2):
                ...
                for _ in range(3):  # three iterations
                    for f in range(3):  # three features per object
                        pooler.compute(
                            feedforwardInputs[obj][f], lateralInputs, learn=True
                        )
            objectRepresentations += [set(pooler.getActiveCells())]
