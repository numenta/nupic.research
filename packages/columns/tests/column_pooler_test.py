...
from nupic.research.frameworks.columns.support.column_pooler_mixin import (
...
class ExtensiveColumnPoolerTest(unittest.TestCase):
    ...
    def testLearnSinglePattern(self):
        ...
        obj = self.generateObject(1)
        self.learn(obj, numRepetitions=2, newObject=True)
        ...
        self.pooler.reset()
        self.infer(feedforwardPattern=obj[0])
        self.assertEqual(
    ...
    def testLearnSingleObject(self):
        ...
        obj = self.generateObject(numPatterns=5)
        self.learn(obj, numRepetitions=2, randomOrder=True, newObject=True)
        representation = self._getActiveRepresentation()
    ...
    def testInferObjectOverTime(self):
        ...
        self.assertEqual(
            set(self.pooler.getActiveCells()), representationA | representationC
        )
    ...
    def testLearnTwoObjectsInTwoColumnsOneCommonPatternEmptyFirstInput(self):
        ...
        for patternsA in objectA:
            ...
            self.assertEqual(desiredRepresentation, self._getActiveRepresentations())
    ...
    def testLateralContestResolved(self):
        ...
        self.learn(
            objectA,
            lateralPatterns=[[lateralInput1A, lateralInput2A]] * 2,
            numRepetitions=3,
            newObject=True,
        )
        representationA = set(self.pooler.getActiveCells())
        self.learn(
            objectB,
            lateralPatterns=[[lateralInput1B, lateralInput2B]] * 2,
            numRepetitions=3,
            newObject=True,
        )
        ...
        self.infer(
            patterns[1], lateralInputs=[lateralInput1A | lateralInput1B, lateralInput2A]
        )
    ...
    def testMultiColumnCompetition(self):
        ...
        self.assertEqual(firstSensedRepresentations, self._getActiveRepresentations())
        ...
        self.assertEqual(secondSensedRepresentations, self._getActiveRepresentations())
        ...
        self.assertEqual(activeRepresentationsA, self._getActiveRepresentations())
    ...
    def testMutualDisambiguationThroughUnions(self):
        ...
        self.assertEqual(firstRepresentations, self._getActiveRepresentations())
    ...
    def setUp(self):
        ...
        self.proximalPatternMachine = PatternMachine(
            n=self.inputWidth, w=self.numOutputActiveBits, num=200, seed=self.seed
        )
    ...
    def learn(
        ...
        for _ in range(numRepetitions):
            ...
            for idx in indices:
                self.pooler.compute(
                    sorted(feedforwardPatterns[idx]),
                    [sorted(lateralPattern) for lateralPattern in lateralPatterns[idx]],
                    learn=True,
                )

    def infer(self, feedforwardPattern, lateralInputs=(), printMetrics=False):
        """
        ...
        """
        self.pooler.compute(
            sorted(feedforwardPattern),
            [sorted(lateralInput) for lateralInput in lateralInputs],
            learn=False,
        )
        ...
        for _ in range(numRepetitions):
            ...
            for i in range(len(indices[0])):
                ...
                for col, pooler in enumerate(self.poolers):
                    ...
                    lateralInputs = [
                        sorted(activeCells)
                        for presynapticCol, activeCells in enumerate(prevActiveCells)
                        if col != presynapticCol
                    ]

                    pooler.compute(
                        sorted(feedforwardPatterns[indices[col][i]][col]),
                        lateralInputs,
                        learn=True,
                    )
    ...
    def inferMultipleColumns(
        ...
        for col, pooler in enumerate(self.poolers):
            ...
            lateralInputs = [
                sorted(activeCells)
                for presynapticCol, activeCells in enumerate(activeRepresentations)
                if col != presynapticCol
            ]
