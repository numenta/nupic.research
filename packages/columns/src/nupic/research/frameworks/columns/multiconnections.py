...
class Multiconnections(object):
    ...
    def computeActivity(self, activeInputsBySource, permanenceThreshold=None):
        ...
        for source, connections in self.connectionsBySource.items():
            o = connections.computeActivity(
                activeInputsBySource[source], permanenceThreshold
            )
            if overlaps is None:
                overlaps = o
            else:
    ...
    def growSynapses(self, segments, activeInputsBySource, initialPermanence):
        ...
        for source, connections in self.connectionsBySource.items():
            connections.growSynapses(
                segments, activeInputsBySource[source], initialPermanence
            )
    ...
    def setPermanences(self, segments, presynapticCellsBySource, permanence):
        ...
        for source, connections in self.connectionsBySource.items():
            if source in presynapticCellsBySource:
                connections.matrix.setElements(
                    segments, presynapticCellsBySource[source], permanences
                )
