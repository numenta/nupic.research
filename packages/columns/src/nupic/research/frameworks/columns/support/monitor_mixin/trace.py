...
class Trace(object, metaclass=abc.ABCMeta):
    ...
    def prettyPrintTitle(self):
        return (
            "[{0}] {1}".format(self.monitor.mmName, self.title)
            if self.monitor.mmName is not None
            else self.title
        )
...
class MetricsTrace(Trace):
    ...
    def prettyPrintDatum(datum):
        return (
            "min: {0:.2f}, max: {1:.2f}, sum: {2:.2f}, "
            "mean: {3:.2f}, std dev: {4:.2f}"
        ).format(datum.min, datum.max, datum.sum, datum.mean, datum.standardDeviation)
