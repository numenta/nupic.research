...
from nupic.research.frameworks.columns.support.monitor_mixin.monitor_mixin_base import (
    MonitorMixinBase,
)
...
class TemporalMemoryMonitorMixin(MonitorMixinBase):
    ...
    def mmGetMetricFromTrace(self, trace):
        ...
        """
        return Metric.createFromTrace(
            trace.makeCountsTrace(), excludeResets=self.mmGetTraceResets()
        )
    ...
    def mmGetMetricSequencesPredictedActiveCellsPerColumn(self):
        ...
        for predictedActiveCells in list(
        ...
        for column in columns:
            ...
            for cell in cells:
                ...
                for seg in self.connections.segmentsForCell(cell):
                    ...
                    synapseList.sort()
                    synapseStringList = [
                        "{0:3}={1:.2f}".format(sourceCell, permanence)
                        for sourceCell, permanence in synapseList
                    ]
    ...
    def mmPrettyPrintSequenceCellRepresentations(self, sortby="Column"):
        ...
        for sequenceLabel, predictedActiveCells in iter(
            self._mmData["predictedActiveCellsForSequence"].items()
        ):
            cellsForColumn = self.mapCellsToColumns(predictedActiveCells)
    ...
    def mmGetDefaultTraces(self, verbosity=1):
        ...
        traces += [self.mmGetTraceNumSegments(), self.mmGetTraceNumSynapses()]
    ...
    def mmGetCellActivityPlot(
        self, title="", showReset=False, resetShading=0.25, activityType="activeCells"
    ):
        """
        ...
        return self.mmGetCellTracePlot(
            cellTrace,
            self.numberOfCells(),
            activityType,
            title,
