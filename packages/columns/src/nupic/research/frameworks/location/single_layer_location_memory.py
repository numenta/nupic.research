...
class SingleLayerLocationMemory(object):
    ...
    def __init__(
        ...
        self.internalConnections = SparseMatrixConnections(cellCount, cellCount)
        ...
        if len(newLocation) > 0:
            ...
            if learn:
                ...
                self._learnFeatureLocationPair(
                    newLocation, featureLocationInput, featureLocationGrowthCandidates
                )
    ...
    def _learnTransition(self, prevActiveCells, deltaLocation, newLocation):
        ...
        deltaPotentialOverlaps = self.deltaConnections.computeActivity(deltaLocation)
        ...
        for learningSegments in (
            learningActiveDeltaSegments,
        ...
        if self.maxSynapsesPerSegment != -1:
            numNewLocationSynapses = min(
                numNewLocationSynapses, self.maxSynapsesPerSegment
            )
            numNewDeltaSynapses = min(
                numNewLocationSynapses, self.maxSynapsesPerSegment
            )
    ...
    def _learnFeatureLocationPair(
        self, newLocation, featureLocationInput, featureLocationGrowthCandidates
    ):
        """
        ...
        cellsForCandidateSegments = self.featureLocationConnections.mapSegmentsToCells(
            candidateSegments
        )
        ...
        newSegments = self.featureLocationConnections.createSegments(newSegmentCells)
    ...
    def _learn(
        ...
        connections.growSynapsesToSample(
            learningSegments, growthCandidates, maxNew, initialPermanence, rng
        )
