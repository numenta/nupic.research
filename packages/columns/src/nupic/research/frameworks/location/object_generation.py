        ...
        objects.append(
            {
                "features": [
                    {
                        "left": (pos % objectWidth) * featureScale,
...
def generateObjectFeatures(numObjects, featuresPerObject, numFeatures, distribution):
    if distribution == "AllFeaturesEqual_Replacement":
        return np.random.randint(
            numFeatures, size=(numObjects, featuresPerObject), dtype=np.int32
        )
        ...
        return np.array(
            [
                [
                    getRandomFeature(probabilityByFeature)
        ...
        secondBinBegin = int(math.ceil(numFeatures / 2.0))
        return np.array(
            [
                [np.random.randint(secondBinBegin)]
                + [
...
def generateObjects(
    ...
    objectFeatures = generateObjectFeatures(
        numObjects, featuresPerObject, numFeatures, distribution
    )
