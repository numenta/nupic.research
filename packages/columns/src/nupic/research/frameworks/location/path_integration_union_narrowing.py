...
def computeRatModuleParametersFromReadoutResolution(
    inverseReadoutResolution, enlargeModuleFactor=1.0
):
    """
    Compute 'cellsPerAxis', 'bumpSigma', and 'activeFiringRate' parameters for
    ...
    readoutResolution = 1.0 / (enlargeModuleFactor * inverseReadoutResolution)
    activeFiringRate = (
        ThresholdedGaussian2DLocationModule.chooseReliableActiveFiringRate(
...
def createRatModuleFromReadoutResolution(
    inverseReadoutResolution, scale, enlargeModuleFactor=1.0, fixedScale=False, **kwargs
):
    """
...
class PIUNCorticalColumn(object):
    ...
    def __init__(self, locationConfigs, L4Overrides=None, bumpType="gaussian"):
        ...
        if bumpType == "gaussian":
            self.L6aModules = [
                createRatModuleFromCellCount(anchorInputSize=L4cellCount, **config)
            ...
            self.L6aModules = [
                Superficial2DLocationModule(anchorInputSize=L4cellCount, **config)
                for config in locationConfigs
            ]
        else:
    ...
    def getLearnableLocationRepresentation(self):
        ...
        for module in self.L6aModules:
            learnableCells = np.append(
                learnableCells, module.getLearnableCells() + totalPrevCells
            )
            totalPrevCells += module.numberOfCells()
...
class PIUNExperiment(object):
        ...
        self.features = dict(
            (
                ...
                np.array(
                    sorted(
                        random.sample(
                            range(self.column.L4.numberOfColumns()),
                            self.numActiveMinicolumns,
                        )
                    ),
                    dtype="uint32",
                ),
    ...
    def learnObject(
        ...
        for _ in range(numIters):
            for iFeature, feature in enumerate(objectDescription["features"]):
                ...
                ].append(locationRepresentation)
                self.inputRepresentations[
                    (objectDescription["name"], iFeature, feature["name"])
                ] = self.column.L4.getWinnerCells()

                locationTuple = tuple(locationRepresentation)
                locationsAreUnique = (
                    locationsAreUnique and locationTuple not in self.representationSet
                )
    ...
    def inferObjectWithRandomMovements(
        ...
        for _ in range(self.maxTraversals):
            ...
            for iFeature in touchSequence:
                ...
                if not inferred:
                    # Use the sensory-activated cells to detect whether the
                    # object has been recognized. In some models, this set of
                    # cells is equivalent to the active cells. In others, a set
                    # of cells around the sensory-activated cells become active.
                    # In either case, if these sensory-activated cells are
                    # correct, it implies that the input layer's representation
                    # is classifiable -- the location layer just correctly
                    # classified it.
                    representation = (
                        self.column.getSensoryAssociatedLocationRepresentation()
                    )
    ...
    def _move(self, feature, randomLocation=False, useNoise=True):
        ...
        if randomLocation:
            ...
            locationOnObject = {
                "top": feature["top"] + feature["height"] / 2.0,
                "left": feature["left"] + feature["width"] / 2.0,
            }

        if self.locationOnObject is not None:
            displacement = {
                "top": locationOnObject["top"] - self.locationOnObject["top"],
                "left": locationOnObject["left"] - self.locationOnObject["left"],
            }
            if useNoise:
                params = self.column.movementCompute(
                    displacement, self.noiseFactor, self.moduleNoiseFactor
                )
            else:
    ...
    def _sense(self, featureSDR, learn, waitForSettle):
        ...
        while True:
            (inputParams, locationParams) = self.column.sensoryCompute(
                featureSDR, learn
            )
