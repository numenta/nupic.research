...
class SpatialPooler:
    def __init__(
        ...
        self.num_active_minicolumns_per_inh_area = int(
            num_active_minicolumns_per_inh_area
        )
        self.local_density = local_density

        assert (self.num_active_minicolumns_per_inh_area > 0) or (
        ...
        self.potential_pools = np.zeros(
            (self.num_minicolumns, self.num_inputs), dtype=np.bool_
        )
        self.permanences = np.zeros(
            (self.num_minicolumns, self.num_inputs), dtype=real_type
        )
        ...
        for minicolumn_index in range(self.num_minicolumns):
            ...
            self.update_permanences_for_minicolumn(
                permanence, minicolumn_index, raise_perm=True
            )
    ...
    def map_potential(self, index):
        ...
        selected_inputs = self.generator.choice(
            minicolumn_inputs, size=num_potential, replace=False
        ).astype(uint_type)
    ...
    def map_minicolumn(self, index):
        ...
        minicolumn_coordinates = np.array(
            np.unravel_index(index, self.minicolumn_dims), dtype=real_type
        )
    ...
    def get_minicolumn_neighborhood(self, center_minicolumn):
        ...
        return self.neighborhood(
            center_minicolumn, self.inhibition_radius, self.minicolumn_dims
        )
    ...
    def average_connected_synapses_per_minicolumn(self, minicolumn_index):
        ...
        if connected.size == 0:
            ...
            for i in connected:
                min_coordinate = np.minimum(
                    min_coordinate, np.unravel_index(i, self.input_dims)
                )
                max_coordinate = np.maximum(
                    max_coordinate, np.unravel_index(i, self.input_dims)
                )
    ...
    def update_permanences_for_minicolumn(
        self, permanence, minicolumn_index, raise_perm=True
    ):
        """
        ...
        permanence[permanence < self.synapse_perm_trim_threshold] = 0
        np.clip(
            permanence, self.synapse_perm_min, self.synapse_perm_max, out=permanence
        )
    ...
    def bump_up_weak_minicolumns(self):
        ...
        for minicolumn_index in weak_minicolumns:
            ...
            # do the update
            self.update_permanences_for_minicolumn(
                permanence, minicolumn_index, raise_perm=False
            )
    ...
    def update_boost_factors_global(self):
        ...
        if self.local_density > 0:
            ...
            target_density = (
                float(self.num_active_minicolumns_per_inh_area) / inhibition_area
            )
